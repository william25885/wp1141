...
**注意**：資料庫檔案（`*.db`）已被 `.gitignore` 忽略，每個開發者需要自行初始化資料庫。
...
2. **設定環境變數**：
   - 編輯 `backend/.env`，填入您的 `GOOGLE_PLACES_API_KEY`
   - 編輯 `frontend/.env`，填入您的 `VITE_GOOGLE_MAPS_API_KEY`
